...
<template>
  ...
  <div class="bg-white w-96 h-full fixed right-0 top-0 z-20 p-8 dark:bg-slate-700">
    <div class="flex items-center gap-4 mb-5">
      ...
      <h2 class="text-2xl font-bold dark:text-slate-100">Корзина</h2>
    </div>
    ...
    <div v-else>
      ...
      <div v-if="totalPrice" class="flex flex-col gap-4 mb-6 mt-7">
        <div class="flex gap-1">
          <span class="dark:text-slate-100">Налог 5%:</span>
          <div class="flex-1 border-b border-dashed" />
          <b class="dark:text-slate-100">{{ vatPrice }} ₽</b>
        </div>
        <div class="flex gap-1">
          <b class="dark:text-slate-100">Итого:</b>
          <div class="flex-1 border-b border-dashed" />
          <b class="dark:text-slate-100">{{ totalPrice + vatPrice }} ₽</b>
        </div>
        <button
          :disabled="buttonDisabled"
          @click="createOrder"
          class="flex items-center justify-center mt-4 bg-lime-500 w-full rounded-xl py-3 text-white disabled:bg-slate-300 hover:bg-lime-600 active:bg-lime-700 transition cursor-pointer dark:bg-slate-500 dark:hover:bg-slate-600 dark:disabled:bg-slate-300 dark:active:bg-slate-700"
        >
